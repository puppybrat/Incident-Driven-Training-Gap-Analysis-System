...
namespace Incident_Driven_Training_Gap_Analysis_System.Data
{
    public class ReferenceDataRepository
    {
        ...
        /// Used on startup to populate the database with default reference data if it is empty. This method checks for the presence of reference data and inserts default records if necessary. Returns true if reference data is available or was seeded during this call, or false if data was unavailable and seeding failed.
        /// </summary>
        /// <returns>true if database is ready; otherwise, false.</returns>
        public bool SeedReferenceDataIfNeeded()
        {
            try
            {
                ...
                // Checks if data already exists. One table is checked as a representative sample, but any could be used. This is a simple and efficient way to determine if the database has already been seeded with reference data, as the presence of any records in the Line table indicates that the necessary reference data has already been inserted.
                // Returns true if data is present, allowing the application to proceed without attempting to insert duplicate reference data. If no records are found, the method continues to seed the database with default reference data.
